...
import { donors as _donors, pickups as _pickups } from '../data/mockData'
...
export default function Dashboard({ onNav }) {
  ...
  useEffect(() => {
    fetchDashboardStats().then(d => {
      setData(d)
      // Also get live donor/pickup data for tables
      // Using mock arrays directly (same source as api.js mock)
      setDonors(_donors)
      setPickups(_pickups)
    })
  }, [])

  if (!data) return <div className="page-body"><div className="empty-state"><p>Loading dashboard…</p></div></div>

  const { stats, monthlyData, itemBreakdown } = data
    ...
    <div className="page-body">
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card yellow">
          ...
          {/* Clarified: this is kabadiwala→FP payment, not donor payment */}
        ...
        <div className="stat-card orange">
          ...
          {/* Kabadiwala pending payments, not donor */}
      ...
      <div className="two-col">
        ...
        <div className="card page-section">
          <div className="card-header">
            ...
            <div className="card-title">Monthly RST Value — Kabadiwala Payments (₹)</div>
          </div>
      ...
      <div className="two-col">
            ...
            {upcoming.length === 0 ? (
              ...
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', borderBottom: '1px solid var(--border-light)' }}>
                ...
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  <span className="badge badge-primary" style={{ fontSize: 10 }}>{p.type}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="card">
          <div className="card-header">
            ...
            {/* Clearly labelled as kabadiwala payments owed TO FreePathshala */}
            ...
            <tbody>
              {donors.filter(d => d.status !== 'Lost').map(d => (
                <tr key={d.id}>
                  ...
                  <td style={{ fontSize: 12.5, fontWeight: new Date(d.nextPickup) < new Date() ? 700 : 400, color: new Date(d.nextPickup) < new Date() ? 'var(--danger)' : 'inherit' }}>
                    {fmtDate(d.nextPickup)}
                  </td>
                  {/* totalRST = scrap value of items donated, not money to donor */}
