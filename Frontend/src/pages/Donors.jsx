...
import {
  ...
  ChevronDown, Download, AlertTriangle, Filter, SlidersHorizontal,
} from 'lucide-react'
import { fetchDonors, createDonor, updateDonor, deleteDonor } from '../services/api'
import {
...
export default function Donors({ triggerAddDonor, onAddDonorDone }) {
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        const updated = await updateDonor(editing.id, form)
        setDonors(ds => ds.map(d => d.id === editing.id ? updated : d))
      } else {
        const newD = await createDonor(form)
        setDonors(ds => [newD, ...ds])
      }
      closeModal()
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'nowrap', marginBottom: 8 }}>
          ...
          >
            ...
            {hasFilters ? <span style={{ background: 'var(--primary)', color: '#fff', borderRadius: '50%', width: 16, height: 16, fontSize: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', marginLeft: 2 }}>{[filterCity, filterSector, filterStatus, filterSociety].filter(Boolean).length}</span> : 'Filter'}
          </button>
        ...
        {/* Row 2: Filters (collapsible, 2-column grid on mobile) */}
        {showFilters && (
          <div style={{
            display: 'grid',
            ...
            <select
              ...
              style={{ fontSize: 12, height: 34, borderRadius: 6, padding: '0 8px' }}
            >
              ...
              <div key={d.id} className="card" style={{ overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '14px 16px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{
                    ...
                    justifyContent: 'center', fontFamily: 'var(--font-display)', fontSize: 17,
                ...
                <div style={{
                  ...
                  {[
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ fontSize: 9.5, color: overdue && item.label.includes('Overdue') ? 'var(--danger)' : 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{item.label}</div>
                    </div>
                  ))}
                </div>
            ...
            <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <input value={form.house} onChange={e => setField('house', e.target.value)} placeholder="e.g. A-101, Flat 3B" />
                </div>
                <div className="form-group">
                  <label>Society</label>
