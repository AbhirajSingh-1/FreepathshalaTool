...
export default function PickupScheduler() {
  ...
  useEffect(() => {
    setTabLoading(true)
    const t = setTimeout(() => { setTabData(schedulerPickups); setTabLoading(false) }, 800)
    return () => clearTimeout(t)
  }, [])
  ...
  const handleSchedule = async () => {
    ...
    const donor = activeDonors.find(d => d.id === selectedDonor)
    ...
    setSelectedDonor(''); setDate(''); setTimeSlot(''); setNotes('')
    setSaving(false)
    setToast('Pickup scheduled successfully!')
  }

  const donorDetails = activeDonors.find(d => d.id === selectedDonor)
    ...
    <div className="page-body">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {PICKUP_MODES.map(mode => (
                    <button key={mode} type="button" onClick={() => handleModeChange(mode)}
                      style={{
                        ...
                        fontWeight: pickupMode === mode ? 700 : 400, cursor: 'pointer',
                      }}>
                      {mode}
                    </button>
                  ))}
              ...
              <div className="form-group" style={{ margin: 0 }}>
                ...
                <textarea value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="Any instructions for the pickup team…" style={{ minHeight: 68 }} />
              </div>
      ...
      <div className="card">
        <div className="card-header">
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Click a section to view details</div>
        </div>
        <div className="card-body">
          <PickupTabs activeTab={activeTab} onTabChange={setActiveTab} data={tabData} loading={tabLoading} />
        </div>
      </div>

      {showDonorModal && (
        <DonorModal onClose={() => setDonorModal(false)} onAdd={handleAddDonor} />
      )}
